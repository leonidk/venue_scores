...
def context_iter(dblp_path):
    ...
    return etree.iterparse(source=gzip.GzipFile(dblp_path,'rb'), dtd_validation=True, load_dtd=True)  # required dtd
...
def parse_entity(dblp_path, save_path, type_name, features=None, save_to_csv=False, include_key=False):
    ...
    if save_to_csv:
        ...
        import pickle
        with open('articles.pkl', mode='wb') as f:
        #with codecs.open('articles.pkl', mode='wb', encoding='utf8', errors='ignore') as f:
...
def parse_publications(dblp_path, save_path, save_to_csv=False, include_key=True):
    type_name = ['article', 'book', 'incollection', 'inproceedings']
...
def main():
    dblp_path = './dblp-2019-01-01.xml.gz'
