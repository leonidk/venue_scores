...
#    gz = gzip.GzipFile('dblp-original.xml.gz')
    gz = gzip.GzipFile('dblp-2019-01-01.xml.gz')
    xmltodict.parse(gz, encoding='utf-8',item_depth=2, item_callback=handle_article)
    ...
    try:
        ...
        if 'author' in article:
            ...
            for authorName in authorList:
                if authorName == 'Angela Dai':
                    print(authorList)
                if authorName == 'Leonid Keselman':
                    print(article)
        ...
        if confname in confdict:
            ...
            areaname = 'unknown'#confdict[confname]
            #return True
    ...
    if countPaper(confname, year, volume, number, pages, startPage, pageCount, url, title):
        ...
        for authorName in authorList:
            ...
            if realName in facultydict:
                ...
                facultydict[realName] = 'Unknown'
            names.append(foundAuthor.encode('utf-8'))
        if True:
            log = { 'name' : names,
                    'year' : year,
...
def dump_it():
    ...
    import bz2
    import pickle
    with bz2.BZ2File('main_log.pkl.bz2','wb') as fp:
        pickle.dump(main_log,fp,-1)
    #df = pd.DataFrame(main_log)
    #df.to_pickle('papers.pkl.bz2')
